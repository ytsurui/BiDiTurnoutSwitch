...
uint16_t trainAddrCache;
uint8_t trainAddrRecvFlag;
	...
	if (recvPos == 2) {
		...
		if (ID == 0x01) {
			...
			if (checkRecvAddrCache(recvAddr)) {
				...
				if (trainAddrCache != recvAddr) {
					trainAddrCache = recvAddr;
					trainAddrRecvFlag |= 0x01;
				}
...
uint8_t trainAddrRecvStat(void) {
	if (trainAddrRecvFlag & 0x01) {
		trainAddrRecvFlag &= 0xFE;
		return (1);
	}
	return (0);
}
